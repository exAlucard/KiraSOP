...
bot_flags = {
    ...
    # v8: быстрый обычный поиск цели кнопками 5 / 6.
    # При ON targeting.py отправляет примерно 10 команд поиска в секунду,
    # пока HP-полоска цели не появится.
...
def create_pause_overlay(root, pause_event, hud_settings_module):
    ...
    # v16: два независимых сценария anti-aggro живут именно в активном config_*.py.
    # Если старый конфиг ещё не содержит переменные — один раз добавляем defaults.
    scenario_defaults = {
    ...
    bot_flags['anti_agr_kill'] = bool(getattr(config, 'THREAT_SCENARIO_KILL_ENABLED', True))
    bot_flags['anti_agr_full_hp'] = bool(getattr(config, 'THREAT_SCENARIO_FULL_HP_ENABLED', True))
    bot_flags['anti_agr'] = bool(bot_flags['anti_agr_kill'] or bot_flags['anti_agr_full_hp'])
    ...
    def _sync_antiaggro_scenario_flags():
        kill_enabled = bool(getattr(config, 'THREAT_SCENARIO_KILL_ENABLED', True))
        full_enabled = bool(getattr(config, 'THREAT_SCENARIO_FULL_HP_ENABLED', True))
    ...
    def toggle_antiaggro_scenario(flag_key, config_name, button_widget):
        # v16.1: UI должен переключаться мгновенно и не зависеть от записи файла.
        # Раньше сначала выполнялся save_config_values(), и при любой ошибке записи
        # callback выходил до обновления bot_flags/текста кнопки. В результате кнопка
        # визуально оставалась ON. Теперь runtime-состояние меняем первым.
        current = bool(bot_flags.get(flag_key, getattr(config, config_name, True)))
        ...
        # Меняем активный config-модуль напрямую. Это сразу видит ConfigProxy и
        # is_flag_enabled(), даже если запись config_*.py временно не удалась.
        try:
        ...
        # Обновляем кнопку ДО файлового I/O.
        update_flag_button_style(flag_key, button_widget)

        # Дублируем состояние в hud_settings как резервное UI-состояние.
        try:
            la2_bot.config.hud_settings.save_button_state(
                flag_key,
                new_value,
        ...
        # Persist в config_*.py. Ошибка записи больше не откатывает runtime/UI.
        try:
            save_config_values({config_name: new_value}, create_backup=False)
        except Exception as exc:
            print(f"[Overlay] Ошибка сохранения {config_name} в config-файл: {exc}")

        # Синхронизируем master-флаг после runtime update.
        _sync_antiaggro_scenario_flags()
        update_flag_button_style(flag_key, button_widget)

        # Если панель «Конфиг» уже открыта, сразу обновим и её.
        try:
            if config_instance is not None and tk.Toplevel.winfo_exists(config_instance.root):
    ...
    anti_coin_btn = tk.Button(
        flags_frame,
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("anti_coin", anti_coin_btn),
    )
    ...
    # v8: новая кнопка сразу под выбором 1/2 целей.
    rapid_target_search_btn = tk.Button(
        flags_frame,
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag(
            "rapid_target_search",
            rapid_target_search_btn,
        ),
    )
    rapid_target_search_btn.pack(side=tk.TOP, anchor='w', pady=(0, 2))
    update_flag_button_style("rapid_target_search", rapid_target_search_btn)

    # Local Altheal worker removed; handled in engine via Arduino.
    altheal_btn = tk.Button(
        flags_frame,
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("altheal", altheal_btn),
    )
    ...
    buffs_btn = tk.Button(
        ...
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("buffs", buffs_btn),
    )
    ...
    heal_btn = tk.Button(
        ...
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("heal", heal_btn),
    )
    ...
    altds_btn = tk.Button(
        ...
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("altds", altds_btn),
    )
    ...
    flip_btn = tk.Button(
        ...
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("flip", flip_btn),
    )
    flip_btn.pack(side=tk.TOP, anchor='w', pady=(0, 2))
    update_flag_button_style("flip", flip_btn)

    vkatak_btn = tk.Button(
        flags_frame,
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("vkatak", vkatak_btn),
    )
    ...
    stuck_btn = tk.Button(
        ...
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("stuck", stuck_btn),
    )
    stuck_btn.pack(side=tk.TOP, anchor='w', pady=(0, 2))
    update_flag_button_style("stuck", stuck_btn)

    anti_no_target_btn = tk.Button(
        flags_frame,
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("anti_no_target", anti_no_target_btn),
    )
    ...
    poke_btn = tk.Button(
        ...
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("poke", poke_btn),
    )
    poke_btn.pack(side=tk.TOP, anchor='w', pady=(0, 2))
    update_flag_button_style("poke", poke_btn)

    always_assist_btn = tk.Button(
        flags_frame,
        font=("Arial", 9),
        width=button_width,
        command=lambda: toggle_flag("always_assist", always_assist_btn),
    )
    ...
    def toggle_config_panel():
        ...
        try:
            exists = config_instance is not None and tk.Toplevel.winfo_exists(config_instance.root)
        except Exception:
            exists = False

        if not exists:
            config_instance = create_config_panel(root, current_client_name)
        else:
...
def is_flag_enabled(flag_name):
    # v16: anti-aggro сценарии читаются из активного config_*.py, поэтому
    # изменения через панель «Конфиг» начинают действовать сразу.
    if flag_name == 'anti_agr_kill':
        return bool(getattr(config, 'THREAT_SCENARIO_KILL_ENABLED', True))
    if flag_name == 'anti_agr_full_hp':
        return bool(getattr(config, 'THREAT_SCENARIO_FULL_HP_ENABLED', True))
    if flag_name == 'anti_agr':
        return bool(
            getattr(config, 'THREAT_SCENARIO_KILL_ENABLED', True)
            or getattr(config, 'THREAT_SCENARIO_FULL_HP_ENABLED', True)
        )
