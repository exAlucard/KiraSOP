    ...
    def return_to_target_logic():
        ...
        try:
            while not return_to_target_stop_event.is_set() and pause_event.is_set():
                ...
                # Пока anti-aggro наблюдает, захватывает или защищает агрессора,
                # RETURN_TO_TARGET не должен перебивать ближайшую цель.
                if is_threat_watcher_active() or is_anti_aggro_priority_active():
                    log_event_throttled(
                        "return_target_paused_antiaggro", 1.0,
                        "RETURN_TO_TARGET_PAUSED_BY_ANTIAGGRO", level="debug",
                ...
                if wait_start_time is None:
                    wait_time = random.uniform(
                        config.HP1_MONSTER_WAIT_TIME_MIN,
                        config.HP1_MONSTER_WAIT_TIME_MAX,
                    )
                    wait_start_time = time.time()
                    print(
                        f"[return_to_target] Пиксель 1 ХП не красный. "
                        f"Начало ожидания {wait_time:.2f} секунд."
                    )

                elapsed_time = time.time() - wait_start_time
                if elapsed_time >= wait_time:
                    # Повторная проверка непосредственно перед командой закрывает
                    # маленькое окно гонки между timeout и send_command.
                    if is_threat_watcher_active() or is_anti_aggro_priority_active():
                        wait_start_time = None
                        continue

                    print(
                        "[return_to_target] Время ожидания истекло. "
                        "Выполняю возврат к цели (RETURN_TO_TARGET)."
                    )
                    send_command(ser, 'RETURN_TO_TARGET')
                    pause_time = random.uniform(
                        config.RETURN_TO_TARGET_PAUSE_MIN,
                        config.RETURN_TO_TARGET_PAUSE_MAX,
                    )
