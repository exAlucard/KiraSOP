...
def use_hp_potion_if_needed(ser, last_potion_time):
    """Использует HP-банку при низком HP с точным монотонным кулдауном."""
    ...
    if (
        not is_color_match(color, config.CHAR_HP_COLOR)
        and now - last_potion_time >= config.POTION_INTERVAL
    ):
        # Не запускаем кулдаун, если команда по какой-то причине не ушла.
        if send_command(ser, 'HP_POTION'):
            print("Пью банку ХП..")
            return now
